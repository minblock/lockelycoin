...
const char * const BITCOIN_CONF_FILENAME = "lockelycoin.conf";
...
#else
    const char* pszModule = "lockelycoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "lockelycoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/lockelycoin";
#else
    // Unix
    return pathRet / ".lockelycoin";
#endif
#endif
}
